...
export default function VoiceInput({ onConfirm }: Props) {
  ...
  async function confirm() {
    ...
    setStatus('saving');
    try {
      await onConfirm({ name: analysis.name.trim(), meal_type: mealType || null, calories: totals.calories, protein: totals.protein, fat: totals.fat, carbs: totals.carbs, weight_grams: effectiveWeight, eaten_at: new Date().toISOString(), notes: null });
  ...
  return <div>
    ...
    {(status === 'preview' || status === 'analysing') && <div className="space-y-3 bg-white dark:bg-gray-800 rounded-2xl p-4 shadow-sm border border-blue-100 dark:border-blue-900 mt-2"><p className="text-xs font-medium text-gray-500 dark:text-gray-400">{t('voice.previewLabel')}</p><textarea value={transcript} onChange={(event) => setTranscript(event.target.value)} disabled={status === 'analysing'} rows={3} className="w-full border border-gray-200 dark:border-gray-600 bg-white dark:bg-gray-700 rounded-lg px-3 py-2 text-sm text-gray-900 dark:text-gray-100 whitespace-pre-wrap disabled:opacity-70" aria-label={t('voice.previewLabel')} />{status === 'analysing' ? <div className="rounded-xl border border-blue-200 bg-blue-50 px-3 py-3 text-blue-950 dark:border-blue-700 dark:bg-blue-950/60 dark:text-blue-100" role="status" aria-live="polite"><div className="flex items-center gap-2 text-sm font-semibold"><Loader2 size={18} className="animate-spin" />{t('voice.waitingForAi')}</div><p className="mt-1 pl-6 text-xs text-blue-800 dark:text-blue-200">{t('voice.waitingHint')}</p><div className="mt-3 h-1.5 overflow-hidden rounded-full bg-blue-200 dark:bg-blue-900" role="progressbar" aria-label={t('voice.waitingForAi')} aria-valuetext={t('voice.analysing')}><div className="h-full w-1/2 animate-pulse rounded-full bg-blue-600" /></div></div> : <><div className="text-xs text-gray-500 dark:text-gray-400">{searching ? <span className="inline-flex gap-1 items-center"><Loader2 size={14} className="animate-spin" />{t('voice.searching')}</span> : results.length ? t('voice.matches') : t('voice.noMatches')}</div>{results.length > 0 && <div className="space-y-2">{results.map((product) => <button key={product.id} type="button" onClick={() => chooseProduct(product)} className="w-full text-left rounded-lg border border-gray-200 dark:border-gray-600 p-2 text-gray-900 dark:text-gray-100 hover:border-blue-400"><span className="block text-sm font-medium">{product.name}</span><span className="text-xs text-gray-500 dark:text-gray-400">{product.calories} {t('macro.calories')} · {t('macro.pAbbr')} {product.protein}{t('macro.g')} · {t('macro.fAbbr')} {product.fat}{t('macro.g')} · {t('macro.cAbbr')} {product.carbs}{t('macro.g')} / 100{t('macro.g')}</span></button>)}</div>}<div className="flex gap-2"><button type="button" onClick={() => { reset(); void startListening(); }} className="flex-1 flex items-center justify-center gap-1.5 border border-gray-200 dark:border-gray-600 rounded-lg py-2 text-sm text-gray-900 dark:text-gray-100"><RotateCcw size={14} />{t('voice.rerecord')}</button>{!searching && results.length === 0 && <button type="button" onClick={() => void analyseTranscript()} disabled={!transcript.trim()} className="flex-1 bg-blue-600 text-white rounded-lg py-2 text-sm font-medium disabled:opacity-50">{t('voice.analyze')}</button>}</div></>}</div>}
    {error && status !== 'review' && status !== 'saving' && <p className="text-sm text-red-500 mt-2 px-1">{error}</p>}
    {(status === 'review' || status === 'saving') && analysis && <div className="bg-white dark:bg-gray-800 rounded-2xl p-4 shadow-sm border border-blue-100 dark:border-blue-900 mt-2 space-y-3"><div className="flex items-center justify-between"><h3 className="font-semibold text-gray-900 dark:text-gray-100 text-sm">{analysis.name}</h3><button onClick={reset} className="text-xs text-gray-500 dark:text-gray-300">{t('voice.retry')}</button></div>{transcript && <p className="text-xs text-gray-500 dark:text-gray-300 whitespace-pre-wrap"><span className="font-medium">{t('voice.transcript')}</span> {transcript}</p>}<label className="block text-xs text-gray-600 dark:text-gray-300">{t('voice.productName')}<input value={analysis.name} onChange={(event) => updateAnalysis('name', event.target.value)} className="mt-1 w-full border border-gray-300 bg-white rounded-lg px-2 py-1 text-sm text-gray-900 dark:border-gray-600 dark:bg-gray-700 dark:text-gray-100" /></label><div className="grid grid-cols-4 gap-2">{([{ key: 'calories', label: t('addMeal.calories') }, { key: 'protein', label: t('macro.protein') }, { key: 'fat', label: t('macro.fat') }, { key: 'carbs', label: t('macro.carbs') }] as const).map(({ key, label }) => <label key={key} className="text-[10px] text-gray-600 dark:text-gray-300">{label}<input type="number" min="0" value={analysis[key]} onChange={(event) => updateAnalysis(key, event.target.value)} className="mt-1 w-full border border-gray-300 bg-white rounded-lg px-1 py-1 text-sm text-gray-900 dark:border-gray-600 dark:bg-gray-700 dark:text-gray-100" /></label>)}</div><p className="text-xs text-gray-600 dark:text-gray-300">{t('voice.per100g')}</p><label className="block text-xs text-gray-600 dark:text-gray-300">{t('voice.weight')}<input inputMode="decimal" value={weight} onChange={(event) => setWeight(event.target.value)} className="mt-1 w-full border border-gray-300 bg-white rounded-lg px-2 py-1 text-sm text-gray-900 dark:border-gray-600 dark:bg-gray-700 dark:text-gray-100" aria-invalid={!parsedWeight} /></label><PortionSelector value={portion} onChange={setPortion} /><div className="flex gap-3 text-xs text-gray-600 dark:text-gray-300"><span className="font-semibold text-gray-900 dark:text-gray-100">{totals?.calories ?? 0} {t('macro.calories')}</span><span>{t('macro.pAbbr')} {totals?.protein ?? 0}{t('macro.g')}</span><span>{t('macro.fAbbr')} {totals?.fat ?? 0}{t('macro.g')}</span><span>{t('macro.cAbbr')} {totals?.carbs ?? 0}{t('macro.g')}</span></div><select className="w-full border border-gray-300 bg-white rounded-lg px-3 py-2 text-sm text-gray-900 dark:border-gray-600 dark:bg-gray-700 dark:text-gray-100" value={mealType} onChange={(event) => setMealType(event.target.value as MealType | '')}><option value="">— {t('addMeal.type')} —</option>{MEAL_TYPE_KEYS.map((meal) => <option key={meal} value={meal}>{t(`addMeal.type.${meal}` as Parameters<typeof t>[0])}</option>)}</select><button onClick={() => void confirm()} disabled={status === 'saving' || !totals || !analysis.name.trim()} className="w-full bg-blue-600 text-white rounded-lg py-2 text-sm font-medium disabled:opacity-50">{status === 'saving' ? t('voice.confirmAdding') : t('voice.confirm')}</button></div>}
  </div>;
}
