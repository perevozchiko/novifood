...
- Добавлена сводка калорий и Б/Ж/У по завтраку, обеду, ужину и перекусам.
- Голосовой ввод перенесён в верхнюю часть дневника.
- Добавление еды по фотографии удалено вместе с его API, клиентским кодом и тестами.
...
- Голосовой ввод блюд (`VoiceInput` компонент): кнопка «Голосовой ввод» на главной странице. Пользователь нажимает кнопку, говорит описание блюда (например, «Йогурт 4% 330 грамм»), речь распознаётся через Web Speech API браузера, текст отправляется в `POST /api/analyze-voice`, Gemini извлекает КБЖУ и возвращает результат. Далее отображается карточка проверки с `PortionSelector` — аналогично фото-анализу (`CameraUpload`).
- `POST /api/analyze-voice` — новый API-роут: принимает `{ text: string }`, использует ту же суточную квоту (50 запросов, таблица `ai_usage`), вызывает Gemini с текстовым промптом.
- `analyzeFoodText(text)` в `src/lib/gemini.ts` — текстовый аналог `analyzeFood`, с той же цепочкой fallback-моделей (`gemini-2.0-flash` → `gemini-1.5-flash` → `gemini-1.5-flash-8b`).
- Новые ключи i18n (`voice.*`) — EN + RU.

## 2026-06-28

Added:

- Кнопка «Удалить все данные» в разделе настроек («Опасная зона»). Перед удалением показывается модальное окно с предупреждением; удаляются записи из таблиц `meals`, `weight`, `water_intake`.

Fixed:

- На мобильных устройствах (iOS Safari) при фокусе на поле ввода (`input`, `select`, `textarea`) больше не происходит авто-масштабирование страницы: установлен `font-size: 16px` в глобальных стилях.
- Выпадающие списки (`<select>` и их `<option>`) теперь отображаются в тёмной теме корректно: добавлены CSS-правила для `.dark select option`.
